...
$siswa = tampil("SELECT * FROM `jadwal_pelajaran` WHERE 1");

if (isset($_GET['hapusid_siswa'])) {
    $id_siswa = $_GET['hapusid_siswa'];
    // Lakukan proses hapus data pengguna berdasarkan id
    mysqli_query($db, "DELETE FROM jadwal_pelajaran WHERE id_pelajaran = '$id_siswa'");
...
<div class="content read">
	<h2>View Tabel</h2>
	<a href="tambahdatainformasi.php" class="create-contact">Tambah File</a>
	<table>
        <thead>
            <tr>
                <td>id</td>
        ...
        <tbody>
            ...
            <tr>
                <td><?=$no++;?></td>
                <td><?=$pgn['hari']?></td>
                <td><?=$pgn['nama_pelajaran']?></td>
                <td><?=$pgn['jam']?></td>
                <td>
                    <a href="hapuspelajaran.php?deleteid_pelajaran=<?=$id?>" onclick="return confirm('apakah data ingin di hapus')">Hapus</a> ||
                    <a href="ubahpelajaran.php?id=<?=$id?>">Ubah</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
